...
public class EchoServer {
    ...
    public void start() throws Exception {
        EventLoopGroup group = new LocalEventLoopGroup();  // 创建Event-LoopGroup
        try {
            ServerBootstrap b = new ServerBootstrap();   //❷ 创建Server-Bootstrap
            b.group(group)
                    .channel(LocalServerChannel.class)    //　 ❸ 指定所使用的NIO传输Channel
                    .localAddress(LocalAddress.ANY) // ❹ 使用指定的端口设置套接字地址
